...
إدخال حالات الأفراد؛ يُطبع التقرير الرسمي بالأعداد فقط
...
الرقم العسكري الاسم الرتبة الحالة ملاحظات {list.map((p) => ( {p.military_number ?? "-"} {p.full_name} {p.military_rank ?? "-"} <Select value={entries[p.id]?.status ?? "present"} onValueChange={(v) => setStatus(p.id, v as AttendanceStatus)} disabled={!canEdit || approved} > {ATTENDANCE_STATUSES.map((s) => ( {STATUS_LABEL[s]} ))} <Input value={entries[p.id]?.note ?? ""} onChange={(e) => setEntries((prev) => ({ ...prev, [p.id]: { status: prev[p.id]?.status ?? "present", note: e.target.value }, }))} disabled={!canEdit || approved} placeholder="ملاحظات..." /> ))} 

); })} ملاحظات التقرير <Input value={notes} onChange={(e) => setNotes(e.target.value)} disabled={!canEdit || approved} /> {canEdit && ( 

<Button onClick={() => saveMut.mutate()} disabled={saveMut.isPending || approved}> حفظ {canApprove && report && !approved && ( <Button variant="outline" onClick={() => approveMut.mutate()} disabled={approveMut.isPending}> اعتماد )} {approved && ( 
...
{/ Main stats table /} <table className="w-full border-collapse text-sm mt-2" style={{ border: "1.5px solid #000" }}> م الصنف الإجازات الأذونات الغياب المستشفى الدورة القوة الموجود {sectionTotals.map((s, i) => ( {i + 1} {s.f} {s.c.leave || ""} {s.c.permit || ""} {s.c.absent || ""} {s.c.sick || ""} {s.c.course || ""} {s.c.total || ""} {s.c.present || ""} ))} الإجمالي {grand.leave || 0} {grand.permit || 0} {grand.absent || 0} {grand.sick || 0} {grand.course || 0} {grand.total || 0} {grand.present || 0} {/ Changes section /} 

التغيرات

<table className="w-full border-collapse text-sm" style={{ border: "1.5px solid #000" }}> التغير الاسم الرتبة الوحدة ملاحظات {renderChangeRows("خروج إجازة", changes?.newLeave ?? [], entries)} {renderChangeRows("عودة", changes?.returned ?? [], entries)} {renderChangeRows("غياب جديد", changes?.newAbsent ?? [], entries)} {renderChangeRows("مريض جديد", changes?.newSick ?? [], entries)} {renderChangeRows("إذن", changes?.newPermit ?? [], entries)} {renderChangeRows("دورة", changes?.newCourse ?? [], entries)} {(!changes || (changes.newLeave.length + changes.returned.length + changes.newAbsent.length + changes.newSick.length + changes.newPermit.length + changes.newCourse.length === 0)) && ( لا توجد تغيرات )} 
...
{@media print { @page { size: A4; margin: 10mm; } body * { visibility: hidden; } .official-report, .official-report * { visibility: visible; } .official-report { position: absolute; inset: 0; margin: 0 auto; } } .official-report th, .official-report td { border: 1px solid #000; padding: 6px 8px; }} 
...
{label} {p.full_name} {p.military_rank ?? "-"} {p.formation ?? "-"} {entries[p.id]?.note ?? ""} 
