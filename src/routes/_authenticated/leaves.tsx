...
function LeavesPage() {
  ...
  const [previewLeave, setPreviewLeave] =
    useState<LeaveRow | null>(null);
    ...
    queryFn: async () => {
      ...
      const { data, error } =
        await supabase
          .from("persons")
          .select(
            "id,full_name,military_rank,squad,military_number,formation"
          )
          .eq("active", true);
  ...
  // جلب الإجازات بدون الاعتماد على الربط الخارجي
  ...
  .filter((id): id is string => !!id);
      ...
      return (leaveData ?? []).map((item) => ({
        ...
        persons:
          item.person_id
            ? personsMap[item.person_id] ?? null
            : null,
      ...
      <Card>
        ...
        <CardContent className="pt-6">
          ...
          <div className="overflow-x-auto">
            ...
            <Dialog
        open={!!previewLeave}
        onOpenChange={(v)=>!v && setPreviewLeave(null)}
      >

        {previewLeave && (
          <LeavePreview leave={previewLeave}/>
        )}
...
function LeaveForm({
  ...
  const [leave_type,setType]=useState(
    "اعتيادية"
  );
