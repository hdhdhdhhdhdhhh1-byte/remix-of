...
function ServicesLayout() {
  ...
  const canEntry = isAdmin || can("services_entry", "view");
  const canView = isAdmin || can("services_view", "view");
  const tabs = [
    { to: "/services", label: "رفع الخدمات", icon: FilePlus2, exact: true, show: canEntry },
    { to: "/services/view", label: "عرض الخدمات", icon: Eye, exact: false, show: canView },
  ].filter((t) => t.show);
  return (
