...
export const ensureOwner = createServerFn({ method: "POST" }).handler(async () => {
  ...
  const { data: existingRole } = await supabaseAdmin.from("user_roles").select("user_id").eq("role", "owner").maybeSingle();
...
async function assertAdmin(userId: string) {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { data } = await supabaseAdmin.from("user_roles").select("role").eq("user_id", userId).in("role", ["owner", "admin"]);
 ...
 .handler(async ({ data, context }) => {
    ...
    if (error && error.message.includes("already been registered")) {
      ...
      if (existing) {
        ...
        created = retry.data; error = retry.error as any;
      }
    }
    ...
    if (roleError) throw new Error(`فشل حفظ الدور: ${roleError.message}`);
...
export const updateUserPermissions = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => UpdatePermsInput.parse(i)).handler(async ({ data, context }) => {
  ...
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  if (data.role) {
    const { data: existing } = await supabaseAdmin.from("user_roles").select("role").eq("user_id", data.user_id);
